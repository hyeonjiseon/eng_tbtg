...
if simParams.typeOfScenario~=2 % Not traffic trace
    ...
    simValues.Xmax = simParams.roadLength;        % Max X coordinate %기본값 1000m - hj
    ...
    if simParams.typeOfScenario == 1 % Legacy PPP %기본 여기로 들어감 - hj
        simValues.direction = rand(Nvehicles,1) > 0.5; %0에서 1 사이의 값을 vehicle 갯수만큼 뽑은 후 0.5보다 크면 1, 아니면 0 - hj
    ...
    elseif simParams.typeOfScenario == 3 % ETSI Highway high speed
        ...
        %아무튼 1에서 Nlanes*2까지의 정수 값 중 하나가 결정됨 - hj
        ...
        %Y값은 랜덤하게 선택한 레인에 roadWidth 곱한 것 - hj
...
%     end
    ...
    %hyeonji - 속도를 시뮬레이션 시작할 때 일정하게 지정해 주어야 한다.
%     IDm = simValues.maxID;
%     simValues.v = simParams.cV/3.6*ones(IDm,1);
%     simValues.v([IDm/4+1:IDm/4*2], 1) = 72/3.6;
%     simValues.v([(IDm/4*2+1):IDm/4*3]) = 48/3.6;
%     simValues.v([IDm/4*3:IDm]) = 36/3.6;
