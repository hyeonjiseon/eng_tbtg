...
for iPhyRaw=1:length(phyParams.Raw)
    ...
    errorMatrix = errorMatrixRawMax(errorMatrixRawMax(:,4)<phyParams.Raw(iPhyRaw),:); %송신/수신 사이 거리 150미만인 것만, 이게 제대로 전송된 것 - hj
    
    %hyeonji - correctMatrix도 Raw가 150미만인 것 해보자
%     correctMatrix = correctMatrixRawMax(correctMatrixRawMax(:,4)<phyParams.Raw(iPhyRaw),:);
