...
%             rxNeiID = neighborsID(indexVehicleTX, rxNeighindex(1,j)); %neighbors(TXid,find한 값)
%             neiRange = (positionManagement.XvehicleReal(rxNeiID) >= 1000) & (positionManagement.XvehicleReal(rxNeiID) <= 2000);
%             n = n + neiRange;
%         end
%     end
%     NrxNeighbors = n;
    ...
    %rxNeighID = neighborsID(indexVehicleTX,rxNeighindex);
%     testX = (positionManagement.XvehicleReal(rxNeighID) >= 1000) & (positionManagement.XvehicleReal(rxNeighID) <= 2000);
%     NrxNeighbors = nnz(testX);
...
%     testNrxNeighbors = nnz((neighborsID(indexVehicleTX,:)>0).* (neighborsDistance(indexVehicleTX,:) < distance) ...
